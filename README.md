...
The nav-probe controls can expire the app session, log out, or clear storage. Clear storage wipes localStorage and sessionStorage for this origin, then reloads with the current widget runtime config copied into URL params so the widget can bootstrap after storage is wiped. For section H, `public/widget-runtime.json` is still required for the server-side CSP allowlist.
